 ...
 * <ul>
 * <li>
 * {@code Bowl} B is linked to {@code Bowl} A, if the {@code
 * setNextBowl(Bowl)} method of A is called with parameter B. Or more concisely
 * {@code A.setNextBowl(B)}.</li>
 * <li>A <em>chain</em> is a number of {@code Bowls} linked together. Every
 * chain has precisely one {@code Kalaha}, and it is always at the end.</li>
 * <li>A <em>start {@code Bowl}</em> of a chain is the first {@code Bowl} in a chain.</li>
 * <li>A <em>linked chain</em> are two chains with the respective {@code Kalaha}
 * s are linked to the start {@code Bowl} of the other chain.</li>
 * <li>A <em>skip</em> is following a link from a {@code Bowl} to the linked
 * {@code Bowl}.</li>
 * <li>In a linked chain every {@code Bowl} is reachable by starting at any
 * {@code Bowl} and skipping. The {@code Bowl} which is reached starting from a
 * certain {@code Bowl} and skipping a number of times is called a <em>tour</em>
 * . To designate a specific tour we will use the following syntax: The tour of
 * {@code Bowl} after n skips.</li>
 * <li>A <em>interlocking chain</em> is a linked chain where every {@code Bowl}
 * has his opposite {@code Bowl} set.</li>
 * <li>A <em>board</em> is a interlocking chain which is interfaced by mapping
 * the {@code Player}s to the start {@code Bowl} of the chains. </i>
 * </ul>
 * 
 * @author dvberkel
 */
public abstract class BoardFactory {
	/**
	 * Returns a modifiable empty {@code Map<K,V>}. It uses type inference to
	...
	/**
	 * Creates a chain of {@code Bowl}s which ends in a {@code Kalaha}. All
	 * {@code Bowls} except the {@code Kalaha} will
	 ...
	 */
	private static Bowl createChain(Player owner, int numberOfBowls,
			int numberOfStones) {
	...
	/**
	 * Creates two changes with a specified number of {@code Bowls} which are
	 * filled with a number of stones.
	 * 
	 * @param builder
	 *            The {@code MancalaBuilder} which specifies the number of
	 *            {@code Bowls} and number of stones.
	 * @return A {@code Map<Player,Bowl>} which maps a {@code Player} to the
	 *         start {@code Bowl} of the chain which is
	...
	private static Map<Player, Bowl> createChains(MancalaBuilder builder) {
		...
		for (Player player : Player.values()) {
			Bowl chain = createChain(player, builder.getNumberOfBowls(),
					builder.getNumberOfStones());
	 ...
	 * @see org.effrafax.game.mancala.BoardFactory.createChains(MancalaBuilder)
	 */
	private static void linkOpponents(MancalaBuilder builder,
			Map<Player, Bowl> startBowlMap) {
		/*
